...
import { Bell, Wallet, ChartLine, CreditCard, ArrowsLeftRight, Coins, Gear, AddressBook as AddressBookIcon, ShieldCheck, Robot, ClockCounterClockwise, Activity } from '@phosphor-icons/react';
...
import { AIRiskDashboard } from '@/components/enterprise/AIRiskDashboard';
import { AuditLog } from '@/components/enterprise/AuditLog';
import { SystemHealth } from '@/components/enterprise/SystemHealth';
import { AIStrategies } from '@/components/enterprise/AIStrategies';
import {
  generateMockWallets,
  ...
  generateMockAIRiskAlerts,
  generateMockAuditLog,
  generateMockSystemHealth,
  generateMockAIStrategies,
} from '@/lib/mock-data';
...
function App() {
  ...
  const aiRiskAlerts = generateMockAIRiskAlerts();
  const auditLog = generateMockAuditLog();
  const systemHealth = generateMockSystemHealth();
  const aiStrategies = generateMockAIStrategies();
  const unreadCount = notifications.filter(n => !n.read).length;
  const activeRiskAlerts = aiRiskAlerts.filter(a => a.status === 'active').length;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 md:grid-cols-6 lg:w-auto lg:inline-grid lg:grid-cols-12 gap-1">
            ...
            <TabsTrigger value="ai-strategies" className="gap-2">
              <Robot size={18} weight="duotone" />
              <span className="hidden sm:inline">AI Strategies</span>
            </TabsTrigger>
            <TabsTrigger value="security" className="gap-2 relative">
              <ShieldCheck size={18} weight="duotone" />
          ...
          <TabsContent value="ai-strategies" className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-bold">AI-Powered Strategies</h2>
            </div>
            
            <AIStrategies strategies={aiStrategies} />
          </TabsContent>
          ...
          <TabsContent value="security" className="space-y-6">
            ...
            <AIRiskDashboard alerts={aiRiskAlerts} />
          </TabsContent>
